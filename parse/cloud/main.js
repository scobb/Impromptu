...
Parse.Cloud.define("destroyFriendship", function(request, response) {
	...
	Parse.Promise.when([ q1.get(fromId), q2.get(toId) ]).then(
			function(from, to) {
				...
				fromGroups.forEach(function(group) {
					...
					var groupIds = _.map(group.get('friendsInGroup'), function(
							friend) {
						return friend.id;
					})
					console.log('from, groupIds: ' + groupIds);
					if (groupIds.indexOf(to.id) > -1) {
						console.log('Found friend ' + to.id + ' in group '
								+ group.id);
						group.get('friendsInGroup').splice(
								groupIds.indexOf(to.id), 1);
					} else {
				...
				toGroups.forEach(function(group) {
					...
					if (groupIds.indexOf(from.id) > -1) {
						console.log('Found friend ' + from.id + ' in group '
								+ group.id);
						group.get('friendsInGroup').splice(
								groupIds.indexOf(from.id), 1);
					} else {
						console.log('This friend not in group ' + group.id);
					}
				})
				...
				if (fromFriendIds.indexOf(to.id) < 0) {
					...
					console.log(to.id + " is a friend of " + from.id
							+ ". Removing.");
				...
				if (toFriendIds.indexOf(from.id) < 0) {
					...
					console.log(from.id + " is a friend of " + to.id
							+ ". Removing.");
		...
		return q2.find();
			...
			function(pushFriends) {
				...
				Parse.Push.send({
					where : q3,
					data : {
						alert : owner.get("name") + ' just invited you to '
								+ title + ': ' + msg
					}
				}, {
					success : function() {
...
Parse.Cloud.define("addNewEvent", function(request, response) {
	...
	query.get(eventId).then(
			function(event) {
				...
				queries.push(q3.find().then(
						function(pushFriends) {
							...
							return Parse.Push.send({
								where : q3,
								data : {
									alert : owner.get("name")
											+ ' just invited you to ' + title
											+ ': ' + msg
								}
							});
				...
				queries.push(q2.find().then(function(streamFriends) {
...
Parse.Cloud.define("eventCleanup", function(request, response) {
	...
	query.get(eventId).then(function(event) {
		...
		users.forEach(function(user) {
			...
			for (var i = events.length - 1; i >= 0; i--) {
				if (events[i].id == eventId) {
					...
					console.log("Removing an object because "
							+ events[i].id + " == " + eventId);
					events.splice(i, 1);
				}
			}
		...
		return Parse.Promise.when([dest_promise,
		                           Parse.Object.saveAll(users)]);
	}).then(function() {
		response.success('bam');
	}, function(error) {
		response.error(error);
	});
});
...
Parse.Cloud.define("acceptFriendRequest", function(request, response) {
	...
	Parse.Promise.when([ q.get(fromId), q2.get(toId) ]).then(
			function(from, to) {
				var q3 = new Parse.Query('Installation');
